...
const registerUser = async(req,res)=>{
   try {
    ...
    if(password.lengtn < 8){
        return res.json({success:false,message:"Password length should be minimum 8"});
    }
    ...
    try {
     ...
     if(slots_booked[slotDate]){
        if(slots_booked[slotDate].includes(slotTime)){
          return res.json({success:false,message:"Slot is not availabe"});
        }else{
